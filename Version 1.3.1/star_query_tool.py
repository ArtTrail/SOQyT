...
_LOG_PATH = os.path.join(_LOG_DIR, 'soqyt_log.log')
_LOG_KEEP = 5   # total logs to keep (current + 4 archived)

# Rotate: rename current log to soqyt_log_YYYY-MM-DD_HHMMSS.log, then prune oldest
try:
    if os.path.exists(_LOG_PATH):
        import glob as _glob
        ts  = _time.strftime('%Y-%m-%d_%H%M%S')
        os.rename(_LOG_PATH, os.path.join(_LOG_DIR, f'soqyt_log_{ts}.log'))
        # Keep only the (_LOG_KEEP - 1) most recent archived logs
        archived = sorted(_glob.glob(os.path.join(_LOG_DIR, 'soqyt_log_*.log')))
        for old in archived[:max(0, len(archived) - (_LOG_KEEP - 1))]:
            os.remove(old)
except OSError:
    pass
...
def query_nea_by_name(name, status_callback):
    ...
    NEA (Oracle) comparisons are case-sensitive, so we UPPER() both sides
    to handle inputs like 'Wasp-36' matching stored 'WASP-36'.
    """
    if status_callback:
        status_callback("Querying NASA Exoplanet Archive...")
    if '*' in name:
        pattern = name.replace('*', '%').replace("'", "''").upper()
        where = f"UPPER(hostname) LIKE '{pattern}' OR UPPER(pl_name) LIKE '{pattern}'"
        return _nea_query(where, status_callback, top=100)
    safe_upper = name.replace("'", "''").upper()
    where = f"UPPER(hostname) = '{safe_upper}' OR UPPER(pl_name) = '{safe_upper}'"
    results = _nea_query(where, status_callback)
    if not results:
        # Try partial match on hostname
        where2 = f"UPPER(hostname) LIKE '%{safe_upper}%'"
...
class StarQueryApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        left_sb = tk.Scrollbar(left_outer, orient='vertical', bg=PANEL,
                               troughcolor=BG, width=12)
        left_sb.grid(row=0, column=1, sticky='ns')

        left_canvas = tk.Canvas(left_outer, bg=PANEL, bd=0, highlightthickness=0,
        ...
        left.bind('<Enter>', _bind_left_scroll)
        left.bind('<Leave>', _unbind_left_scroll)
    ...
    def _fetch_simbad_detail(self, main_id, basic_text, result_row=None):
        ...
        def _q(adql):
            try:
                ...
                if not r.ok:
                    import re as _re
                    _msg = _re.search(r'value="ERROR">(.*?)</INFO>', r.text or '', _re.S)
                    _log.warning(f'  ↳ SIMBAD detail HTTP {r.status_code}: '
                                 f'{(_msg.group(1).strip() if _msg else r.text[:120]).strip()}')
                    return []
                return r.json().get('data', [])
            except Exception as _exc:
                _log.warning(f'  ↳ SIMBAD detail exception: {_exc}')
                return []
...
""".strip()
        ...
        adql_dist = f"""
SELECT m.dist, m.plus_err, m.unit, m.bibcode
FROM mesDistance m
JOIN basic b ON m.oidref = b.oid
WHERE b.main_id = '{safe_id}'
""".strip()
...
""".strip()
    ...
    def _show_revision_history(self):
        ...
        entry("v1.3.0", "2026-05-23", [
            ...
            "The five most recent session logs are kept as soqyt_log_YYYY-MM-DD_HHMMSS.log.",
            "Log viewer: Help → View Log opens a scrollable, colour-coded log window "
            "with Refresh, Copy All, and Save buttons for easy diagnostics and sharing "
            "with support.",
            ...
            "Fixed two SIMBAD detail query bugs that silently failed for every star: "
            "(1) ORDER BY m.plx_err ASC was rejected by the SIMBAD TAP parser — "
            "parallax measurements were missing from the detail panel; "
            "(2) the distance table was renamed from mesDist to mesDistance (column "
            "dist_err → plus_err) — distance measurements were missing from the "
            "detail panel. Both now return data correctly.",
            "Log viewer error lines are now shown in red; warnings in yellow; "
            "debug lines in muted grey. SIMBAD detail sub-query failures are now "
            "logged as warnings with the server error text instead of being silently "
            "discarded.",
            "Left search-parameter panel is now scrollable — content is no longer "
            "clipped when the window is resized smaller than the panel height.",
        ])
